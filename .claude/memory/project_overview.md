...
├── website/                Marketing site (ignore during extension development)
...
**If a block type doesn't display correctly when opening a file → fix `mdastToLexical.ts`.**  
**If a block type doesn't save correctly to Markdown → fix `lexicalToMdast.ts`.**
