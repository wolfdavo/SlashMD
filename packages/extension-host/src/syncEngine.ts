...
export class SyncEngine implements ISyncEngine {
  ...
  async handleDocumentChange(changes: DocumentChange[]): Promise<void> {
    ...
    try {
      ...
      console.log(
        "[SyncEngine] Handled external document changes, updated to",
        this.currentBlocks.length,
        "blocks"
      );
    } catch (error) {
  ...
  async forceResync(): Promise<void> {
    ...
    try {
      ...
      console.log(
        "[SyncEngine] Force resync completed with",
        this.currentBlocks.length,
        "blocks"
      );
    } catch (error) {
   ...
   */
  private async notifyWebViewOfBlockChanges(
  ...
  async sendInitialBlocks(): Promise<void> {
    ...
    try {
      await this.context.webviewPanel.webview.postMessage(message);
      console.log(
        "[SyncEngine] Sent initial blocks to WebView:",
        this.currentBlocks.length,
        "blocks"
      );
    } catch (error) {
  ...
  private handleSyncConflict(error: SyncConflictError): void {
    ...
    this.forceResync().catch((resyncError) => {
      console.error(
        "[SyncEngine] Failed to resolve sync conflict:",
        resyncError
      );
    });
  }
