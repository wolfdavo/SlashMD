...
import {
  sharedToUIBlocks,
  uiToSharedBlocks
} from "./integration/BlockConverter";
    ...
    <div className="production-controls">
      <div className="phase-info">
        ...
        <span className="phase-description">
          {isVSCodeEnvironment ? "Connected to VS Code" : "Standalone Mode"} •
          React + Lexical + Sync Engine
        </span>
      </div>
          ...
          {showPerformancePanel && (
            <div className="performance-panel">
              <div className="performance-stats">
                <div>
                  Avg Latency: {performanceMetrics.averageLatency.toFixed(1)}ms
                </div>
                ...
                <div>
                  ...
                  <span
                    style={{
                      color: getPerformanceColor(performanceMetrics.status)
                    }}
                  >
...
const EditorWithIntegration: React.FC<{ externalBlocks?: Block[] }> = ({
  externalBlocks
}) => {
  ...
  const suppressOutboundChangeRef = React.useRef(false);
  ...
  // Hydrate editor blocks from externalBlocks (from VS Code) and flip loading flags
  useEffect(() => {
    if (isVSCodeEnvironment && externalBlocks && externalBlocks.length >= 0) {
      suppressOutboundChangeRef.current = true;
      setBlocks(externalBlocks);
      setIsInitialized(true);
      setIsLoading(false);
    }
  }, [isVSCodeEnvironment, externalBlocks, setBlocks]);
  ...
  const handleBlocksReceived = useCallback(
    (sharedBlocks: SharedBlock[]) => {
      console.log(
        "[AppIntegrated] Received blocks from VS Code:",
        sharedBlocks.length
      );
  ...
  // Remove duplicate init requests; rely on VSCodeBridge to handle init once

  // Handle block changes and send to VS Code
  const handleBlocksChange = useCallback(
    (newBlocks: Block[]) => {
      if (suppressOutboundChangeRef.current) {
        suppressOutboundChangeRef.current = false;
        return;
      }
      measureKeystrokeLatency();
      setBlocks(newBlocks);

      // Send debounced BLOCKS_CHANGED to extension
  ...
  const handleAssetRequest = useCallback(
    (dataUri: string, suggestedName?: string, targetBlockId?: string) => {
      console.log("[AppIntegrated] Asset requested:", {
        suggestedName,
        targetBlockId
      });
  ...
  useEffect(() => {
    if (!isVSCodeEnvironment && !isInitialized) {
      setTimeout(() => {
        console.log(
          "[AppIntegrated] Initializing with sample data (standalone mode)"
        );
      ...
      <div className="app">
        ...
        <div className="production-info">
          ...
          <span
            ...
            style={{
              color:
                performanceMetrics.status === "good"
                  ? "#107c10"
                  : performanceMetrics.status === "warning"
                  ? "#ff8c00"
                  : "#d13438"
            }}
          >
...
const AppIntegrated: React.FC = () => {
  ...
  return (
    ...
    >
      <EditorWithIntegration externalBlocks={blocks} />
    </VSCodeBridge>
  );
};
