...
function BlocksPlugin({
  ...
  return (
    <div className="blocks-container">
      {blocks.map((block) => {
        const canIndent =
          block.type === "listItem" || block.type === "taskItem";
        const canOutdent =
          canIndent &&
          "indent" in block.content &&
          (block.content as any).indent > 0;

        return (
          <div
            key={block.id}
            className={`block-container ${getDragClasses(block.id)}`}
          >
            <DragHandle
              blockId={block.id}
...
interface InteractiveLexicalEditorProps {
  ...
  onConvertBlock: (
    blockId: string,
    newType: BlockType,
    options?: { level?: 1 | 2 | 3; ordered?: boolean }
  ) => void;
...
export const InteractiveLexicalEditor: React.FC<
  InteractiveLexicalEditorProps
> = ({
  blocks,
  onChange,
  onConvertBlock,
    ...
    <div className="editor-container" ref={editorRef}>
      <LexicalComposer initialConfig={initialConfig}>
        <div className="editor-root">
          ...
          <HistoryPlugin />
          {false && <OnChangePlugin onChange={() => onChange?.(blocks)} />}
